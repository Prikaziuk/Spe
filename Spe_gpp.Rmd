...
required_packages <- c('knitr', 'REddyProc', 'tidyverse', 'versions', 'lubridate', 'glue', 'digest')
to_install <- required_packages[!(required_packages %in% installed.packages()[,"Package"])]
if(length(to_install)) install.packages(to_install)

if ('tsibble' %in% installed.packages()[,"Package"]) {
  if (packageVersion("tsibble") != '0.8.3') {
    warning('probably downgrading tsibble package')
    versions::install.versions('tsibble', '0.8.3')
  }
} else {
  versions::install.versions('tsibble', '0.8.3')
}
